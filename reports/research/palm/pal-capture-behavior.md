...
**FlowerRabbit/Kitsunebi 집중 파밍의 이유: 단련(enrichment) 재료 소비**

[Fact: ingame_pal_enrich, ingame_pal_capture] 두 팰의 반복 포획은 단련 시스템(팰 강화)의 재료 소비와 직접 연결된다. 단련 시 동종 팰을 3~4마리씩 재료로 소비하는데, 포획 대비 단련 재료 소비 비율을 비교하면 FlowerRabbit/Kitsunebi의 파밍 목적이 분명해진다.

| 팰 | 총 포획 | 단련 재료 소비(추정) | 소비/포획 비율 | 단련 횟수 |
|----|---------|---------------------|---------------|----------|
| FlowerRabbit | 9,424 | 6,916 | **73.4%** | 1,985회 |
| Kitsunebi | 16,606 | 6,135 | **36.9%** | 2,012회 |
| Garm | 10,466 | 6,355 | 60.7% | 2,070회 |
| Penguin | 11,698 | 4,900 | 41.9% | 1,625회 |
| FlameBambi | 11,679 | 4,612 | 39.5% | 1,567회 |
| SheepBall | 11,183 | 1,964 | 17.6% | 706회 |

FlowerRabbit은 포획의 73.4%가 단련 재료로 소비되며, 사실상 "단련 파밍 전용 팰"이다. FlowerRabbit 포획의 99.8%가 Lv15+ 시점에서 발생하는 점도 이를 뒷받침한다 — 초반 수집이 아니라 중반 이후 단련을 위한 의도적 파밍이다. Kitsunebi는 포획량 자체가 1위로 많아 비율(36.9%)은 낮지만, 단련 재료 소비 절대량(6,135)은 FlowerRabbit과 유사하다.

반면 SheepBall은 시도 유저가 가장 많지만(1,801명) 단련 소비 비율은 17.6%에 그치며, "모두가 한 번쯤 잡지만 파밍하지는 않는" 팰이다.
...
| 6 | 포획 총량을 통제해도 다양성-잔존 상관 유지 (단, 저다양성 그룹 n=18로 표본 부족 — 아래 주의) | [Fact] |
...
**1. 포획 다양성은 몰입도의 유의미한 신호이나, 포획 총량 통제 검증은 결론을 확정하기에 부족하다**

Day 1에 다양하게 잡는 유저가 장기 잔존할 가능성이 높다(1~5종 36.1% → 11종+ 82.8%). 포획 총량을 통제(D1 20~40마리)해도 다양성 차이가 유지되는 것처럼 보이지만, **저다양성 그룹(1~7종)이 18명에 불과**하여 이 검증만으로 "총량과 독립적인 효과"를 확정하기 어렵다. 8~12종(66.7%) vs 13종+(71.6%)의 차이도 4.9%p로, 표본 크기 대비 유의미하다고 판단하기 힘들다.

따라서 다양성-잔존 관계는 **탐색적 수준의 상관관계**로, 조기 이탈 예측 지표 후보로서의 가능성은 있으나 CBT에서 더 큰 표본으로 재검증이 필요하다.
...
4. **포획 목적 부분 확인**: v3에서 단련(enrichment) 로그와 교차 분석하여 FlowerRabbit/Kitsunebi 파밍이 단련 재료 소비와 연결됨을 확인했다. 다만 포획→단련 외의 용도(거래, 기지 배치 등)는 여전히 구분이 어렵다.
5. **D1 다양성-잔존 통제 검증의 표본 한계**: 포획량 20~40 구간에서 저다양성 그룹(1~7종)이 18명으로, 이 검증 결과만으로는 "포획 총량과 독립적인 다양성 효과"를 확정할 수 없다. 8~12종 vs 13종+ 차이(4.9%p)도 표본 크기 대비 유의미하다고 보기 어렵다. **다양성-잔존 관계는 탐색적 수준의 발견이며, CBT에서 더 큰 표본으로 재검증이 필요하다.**

### 후속 연구 제안
1. CBT에서 **포획 다양성 지표를 조기 이탈 예측 모델에 포함**하여 예측력 검증 — 특히 포획 총량 통제 검증(반증 2)을 더 큰 표본에서 재수행하여 n=18 한계를 해소
2. ~~반복 포획(파밍) 행동의 목적 분석 — 강화/단련 로그와 연결~~ → v3에서 단련 로그 교차 분석 완료. 단련 외 용도(기지 배치, 거래 등)와의 관계는 추가 분석 필요
...
### v3 (2026-04-13) — 분석 팀장 피드백 반영

**수정 1: FlowerRabbit/Kitsunebi 파밍 이유 연결 (섹션 4.2)**
- ingame_pal_enrich 테이블과 교차 분석하여, 두 팰의 집중 파밍이 단련(enrichment) 재료 소비와 직접 연결됨을 확인
- FlowerRabbit: 포획의 73.4%가 단련 재료로 소비, 포획의 99.8%가 Lv15+ 시점 발생 (단련 파밍 전용)
- Kitsunebi: 단련 재료 소비 절대량 6,135(FlowerRabbit 6,916과 유사), 비율은 36.9%
- 비교 대상으로 SheepBall(17.6%), Garm(60.7%) 등 포함
- 한계 4번 "포획 목적 미구분" → "부분 확인"으로 업데이트, 후속 연구 2번 완료 표시
- 출처: ingame_pal_enrich(enrich_pal_datatable_id, material_count), ingame_pal_capture(is_success=true)

**수정 2: 반증2 n=18 한계를 결론에서 더 솔직히 반영 (섹션 6.1, 6.2)**
- 6.1 확인된 사실 테이블의 6번 항목에 "n=18로 표본 부족" 주의 추가
- 6.2 시사점 1번을 "탐색적 수준의 상관관계"로 톤 조정. "총량과 독립적인 효과를 확정하기 어렵다" 명시
- 한계 5번을 강화: "탐색적 수준의 발견이며 CBT에서 재검증 필요" 명시
- 후속 연구 1번에 "n=18 한계 해소" 목적 명시
