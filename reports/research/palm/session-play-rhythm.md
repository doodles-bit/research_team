...
**Day 3~4 시점의 유저 진행 상태: 콘텐츠 소진 가능성 교차 검증**

급락이 시작되는 Day 3~4에 유저가 어디까지 진행했는지 확인했다. [Fact: ingame_login + ingame_guide_quest, D1 시작 코호트 1,712명, Day 4까지 데이터]

| 레벨 그룹(Day 4 기준) | 유저 수 | D5+ 잔존율 | 가이드 퀘스트 중앙값(quest_idx) |
|----------------------|---------|-----------|------------------------------|
| Lv 1-10 | 713명 (41.6%) | 17.0% | 41 |
| Lv 11-15 | 307명 (17.9%) | 58.3% | 109.5 |
| Lv 16-20 | 282명 (16.5%) | 76.6% | 166 |
| Lv 21-25 | 257명 (15.0%) | 89.5% | 228 |
| Lv 26+ | 153명 (8.9%) | 97.4% | 301 |

D1 코호트의 Day 4 기준 평균 레벨은 12.8, 중앙값 14.0이다. 가이드 퀘스트 진행도 중앙값은 112/363(전체 가이드 퀘스트 기준 30.9%). 즉, Day 4까지 전형적 유저는 **전체 가이드 콘텐츠의 약 1/3을 소화**한 상태다.

주목할 점은 레벨별 잔존율의 극단적 차이다. Lv 1-10(41.6%)은 잔존율이 17.0%에 불과하지만, Lv 21+(23.9%)은 89.5~97.4%가 잔존한다. Day 3~4의 몰입도 급락은 **콘텐츠를 빠르게 소비한 고레벨 유저의 소진**과 **진행이 느린 저레벨 유저의 이탈**이 동시에 작용하는 복합 현상으로 보인다. 다만 5.2절에서 확인했듯이 고레벨 유저도 -46% 감소하므로, 저레벨 이탈만으로 설명되지 않는다.
...
**주의**: 세션 내 이벤트 수 기준의 활동 비중 분석(예: 전투·포획·제작 비율)은 **제작(craft) 이벤트가 전체의 86%를 차지**하므로, 이벤트 수가 곧 시간 투자 비중을 반영한다고 보기 어렵다. 제작은 기지 자동화 등으로 한 번에 대량 발생할 수 있어, 실제 유저의 시간 배분과 괴리가 있을 수 있다.
...
1. **잔존 유저도 일주일 만에 일일 플레이 시간이 66% 감소했다.** Day 3까지는 완만하지만, Day 4부터 급락한다. Day 4 기준으로 전형적 유저(중앙값 Lv14)는 가이드 퀘스트의 약 31%를 소화한 상태이며, Lv 26+(가이드 퀘스트 중앙값 301/363, 83%)까지 진행한 유저(8.9%)는 잔존율 97.4%로 높으나, Lv 1-10(41.6%)은 17.0%에 그친다. 콘텐츠 소진과 진행 정체가 동시에 작용하는 것으로 보인다.
...
- **Day 3~4에 새로운 콘텐츠 동기(motivation) 투입 여부**: 급격한 몰입도 하락이 시작되는 시점이다. Day 4 기준 유저의 가이드 퀘스트 소화율 중앙값이 31% 수준이므로, 콘텐츠가 부족한 것이 아니라 **중반 진행의 동기 부여가 약할 가능성**이 있다. 반면 Lv 26+까지 빠르게 진행한 유저(8.9%)는 전체 콘텐츠의 83%를 소화한 상태로, 이들에게는 실제 소진이 발생했을 수 있다.
...
1. **Day 3~4 시점의 콘텐츠 벽 분석 심화**: 4.3절에서 레벨·퀘스트 진행도 교차 분석을 추가했으나, 구체적으로 어떤 퀘스트/던전에서 정체가 발생하는지(난이도 벽 vs 흥미 저하) 추가 분석이 필요
...
**수정 1: Day 3~4 콘텐츠 소진 가설에 데이터 보강 (섹션 4.3, 6)**
- Day 3~4 시점 유저의 레벨 분포와 가이드 퀘스트 진행도를 교차 분석하여 4.3절에 추가
- D1 코호트 Day 4 기준: 평균 Lv12.8, 중앙값 Lv14, 가이드 퀘스트 중앙값 112/363(31%)
- 레벨별 D5+ 잔존율: Lv1-10 17.0% / Lv11-15 58.3% / Lv16-20 76.6% / Lv21-25 89.5% / Lv26+ 97.4%
- 결론 1번과 시사점 1번도 데이터 반영하여 업데이트
- 후속 연구 1번을 "심화 분석"으로 수정 (기본 교차 분석은 본문에서 완료)
- 출처: ingame_login(user_level), ingame_guide_quest(quest_idx, status='complete'), D1 코호트 1,712명

**수정 2: 제작 이벤트 86% 비중 경고를 본문에 추가 (섹션 4.5)**
- 기존 7절 한계 5번에만 있던 주의사항을 4.5절(레벨 그룹별 세션 프로파일) 뒤에도 추가
- "이벤트 수 기반 활동 비중 분석은 제작 86% 비중으로 시간 투자 비중과 괴리 가능" 주의
